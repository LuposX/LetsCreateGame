...
/** utility class, which detects collisons between different objects. */
public final class CollisionControler {
  /**
  ...
  public static void detectEntityCollisions(ArrayList<Entity> entities, int dt) {
    // Fuer alle Entities (au�er das letzte)
    for (int i = 0; i < entities.size() - 1; i++) {
      Entity e1 = entities.get(i);

      // Fuer alle Entities mit einem Index groe�er als i
  ...
  public static boolean touchesTileLayer(Shape hitbox, int layer) {
    for (int i = 0; i < hitbox.getPointCount(); i++) {
      ...
      if (Controls.map.getHeight() <= posY
          || Controls.map.getWidth() <= posX
          || posX < 0
          || posY < 0) {
        return true;
      }
  ...
  public static boolean isCordInLayer(float posX, float posY, int layer) {
    if (Controls.map.getHeight() <= posY
      ...
      return 0 != Controls.map.getTileId((int) posX, (int) posY, layer);
    }
  }
}
